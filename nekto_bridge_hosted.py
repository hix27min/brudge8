...
# ============================================================
# BROWSER MIC SOURCE — мост между WebRTC от браузера и буфером
# ============================================================
class BrowserMicSource:
    """
    Принимает WebRTC-трек от браузера пользователя и раздаёт
    одни и те же сэмплы трём push-каналам (A, B, self-monitor).
    """
    def __init__(self):
        self.track = None  # RTCRtpReceiver трек от браузера
    ...
    def set_track(self, track):
        """Установить новый входящий трек от браузера."""
        if self._task:
            self._task.cancel()
        self.track = track
        if track is not None:
            self._task = asyncio.create_task(self._pull())
        ...
        try:
            while True:
                ...
                try:
                    ...
                    log.debug(f"browser mic resample: {e}")
                    continue
                for f in out_frames:
                    arr = f.to_ndarray().flatten().astype(np.int16)
                    # RMS для VU
                    ...
                    for sub in self.subscribers:
                        try:
                            sub.push_samples(arr)
                        except Exception:
                            pass
        except asyncio.CancelledError:
            pass
        except Exception as e:
            log.debug(f"browser mic pull ended: {e}")
...
class Bridge:
    def __init__(self):
        ...
        # WebRTC peer connection с браузером (для микрофона и вывода)
        self.user_pc = None
    ...
    async def handle_user_webrtc_offer(self, offer_sdp, offer_type):
        """
        Браузер пользователя присылает offer. Мы отвечаем answer.
        Принимаем его микрофон-трек, отдаём ему out_browser (смешанный A+B).
        """
        if self.user_pc:
            try: await self.user_pc.close()
            except Exception: pass

        self.user_pc = RTCPeerConnection()
        pc = self.user_pc

        @pc.on("track")
        async def on_track(track):
            log.info(f"user browser track: {track.kind}")
            if track.kind == "audio":
                self.browser_mic.set_track(track)

        # подписка на микшер, который шлём юзеру
        browser_track = self.relay.subscribe(self.out_browser)
...
// ===== WebRTC к серверу: шлём микрофон, получаем смешанный звук =====
async function startAudio(){
  ...
  try{
    ...
    userPC=new RTCPeerConnection();
    for(const track of stream.getTracks())userPC.addTrack(track,stream);
    userPC.addTransceiver('audio',{direction:'recvonly'});
    userPC.ontrack=(e)=>{
      document.getElementById('remoteAudio').srcObject=e.streams[0];
      logLine('logMain','получаю звук с сервера','ok');
    };
    userPC.onconnectionstatechange=()=>{
      logLine('logMain','audio: '+userPC.connectionState,
        userPC.connectionState==='connected'?'ok':'');
    ...
    await userPC.setRemoteDescription(ans);
...
async def offer_handler(request):
    ...
    answer = await bridge.handle_user_webrtc_offer(data["sdp"], data["type"])
