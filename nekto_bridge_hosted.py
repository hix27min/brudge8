...
class NektoClient:
    ...
    async def _on_peer_disconnect(self, d):
        # Если мы сами только что скипнули — это просто ACK от сервера
        if getattr(self, '_local_skip_pending', False):
            self._local_skip_pending = False
            self._log("peer-disconnect ACK (after our skip)", "info")
            # освобождаем событие чтобы skip() мог продолжить
            ev = getattr(self, '_skip_ack_event', None)
            if ev: ev.set()
            return
        # Иначе нас скипнули — запускаем новый поиск
        self._log("peer disconnected", "warning")
        await self._close_peer()
        if self.ws and not self.ws.closed and self._should_stay:
            # пауза чтобы сервер успел освободить слот
    ...
    async def skip(self):
        if self.connection_id:
            ...
            self._local_skip_pending = True
            # создаём событие которым сможем дождаться ACK
            self._skip_ack_event = asyncio.Event()
            try:
                await self._emit_event({
                ...
                pass
            await self._close_peer()
            # ждём ACK от сервера максимум 1.5с, иначе идём дальше
            try:
                await asyncio.wait_for(self._skip_ack_event.wait(), timeout=1.5)
                # пришёл ACK — даём серверу ещё 0.7с на освобождение слота
                await asyncio.sleep(0.7)
            except asyncio.TimeoutError:
                self._log("skip ACK timeout, proceeding anyway", "warning")
                await asyncio.sleep(0.5)
            self._skip_ack_event = None
            self._local_skip_pending = False
        else:
...
class PlayerProcessor extends AudioWorkletProcessor{
  constructor(){
    super();
    this.buf=new Float32Array(16000*6); // 6 секунд буфера
    this.writePos=0;
    this.readPos=0;
    this.available=0;
    // preroll: ждём 500мс прежде чем начнём выдавать (для нестабильной сети)
    this.preroll=16000*0.5|0;
    this.prerolled=false;
    // адаптивные пороги
    this.targetBuffer=16000*0.6|0;  // целимся держать ~600мс
    this.maxBuffer=16000*2.0|0;      // > 2с — дропаем (сеть копит)
    this.fadeCounter=0;              // для плавного fade при underrun
    this.port.onmessage=(e)=>{
      ...
      this.available+=n;
      // если буфер слишком разросся (>2с) — дропаем до target
      if(this.available>this.maxBuffer){
        const drop=this.available-this.targetBuffer;
        this.readPos=(this.readPos+drop)%bl;
        this.available-=drop;
      }
    };
  ...
  process(_,outputs){
    ...
    const bl=this.buf.length;
    if(!this.prerolled){
      if(this.available>=this.preroll){
        this.prerolled=true;
        this.fadeCounter=0;
      }else{
        out.fill(0);
    ...
    if(this.available<n){
      // underrun — плавный fade-out в тишину (избегаем щелчков)
      const k=Math.min(this.available,n);
      if(k>0){
        const end=this.readPos+k;
        if(end<=bl){
          out.set(this.buf.subarray(this.readPos,end));
        }else{
          const first=bl-this.readPos;
          out.set(this.buf.subarray(this.readPos),0);
          out.set(this.buf.subarray(0,k-first),first);
        }
        this.readPos=(this.readPos+k)%bl;
        this.available=0;
      }
      // остаток — тишина с плавным fade
      for(let i=k;i<n;i++)out[i]=0;
      // После underrun требуем только МИНИ-preroll чтобы не дёргать слушателя надолго
      this.prerolled=false;
      this.preroll=16000*0.15|0; // после первого — 150мс достаточно
      return true;
    }
